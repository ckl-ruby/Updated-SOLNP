...
def solnp_solver(obj_func, eq_constraints, l_x, u_x, p0, max_outer_iter=100, max_inner_iter=10, tol=1e-6):
    ...
    for k in range(max_outer_iter):
        ...
        for i in range(max_inner_iter):
            ...
            hk = infeas(xi_k, eq_constraints, l_x, u_x)
            ...
            if filter_method_check(fk, hk, filter_set):
                ...
                while alpha > 1e-4:
                    ...
                    hk_temp = infeas(xi_k_temp, eq_constraints, l_x, u_x)
        ...
        if (abs(objective_function(xi_k) - objective_function(p_k)) / max(1, abs(objective_function(p_k))) <= epsilon_s and
            np.linalg.norm(project_to_box(xi_k - approx_fprime(xi_k, lambda x: augmented_lagrangian(x, y, rho, obj_func, eq_constraints), np.sqrt(np.finfo(float).eps)), l_x, u_x) - xi_k) > epsilon_a):
...
p0=np.full(n, 1.2)
# 运行求解器
solution,objective_values, infeasibilities = solnp_solver(objective_function, equality_constraints, l_x, u_x, p0)
